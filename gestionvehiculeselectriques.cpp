...
void GestionVehiculesElectriques::on_pushButton_Open_clicked()
{
    int id=ui->lineEdit_CODE_Alaa->text().toInt();
          if (id==NULL)
          {
          ...
          else if (voiture.rechercherCODE(id))
            {
...
void GestionVehiculesElectriques::update_label() {
    ...
    if (receivedData == "K") {
        ...
        ui->lineEdit_CODE_Alaa->clear();

    } else if (receivedData == "S") {
        // Delete a character from LineEdit_ID
        QString currentText = ui->lineEdit_CODE_Alaa->text();
        if (!currentText.isEmpty()) {
            currentText.chop(1);  // Remove the last character
            ui->lineEdit_CODE_Alaa->setText(currentText);
        }
    } else {
        // Append received data to LineEdit_ID
        ui->lineEdit_CODE_Alaa->setText(ui->lineEdit_CODE_Alaa->text() + receivedData);
    }
}
...
void GestionVehiculesElectriques::on_AjouterVoitureBouton_clicked()
{
    ...
    int CODE=ui->lineEdit_CODE_Alaa->text().toInt();
    // Get the image path
    QString photoPath = QFileDialog::getOpenFileName(this, tr("Choose Photo"), "", tr("Images (*.png *.jpg *.bmp)"));

    VoitureElectrique V(VIN,plaque_immatriculation,marque,modele,etat,charge,maintenance,disponibilite,annee_fabrication,photoPath,CODE);
...
void GestionVehiculesElectriques::on_ModifierVoitureBouton_clicked()
{
        ...
        int newCODE = ui->lineEdit_CODE_Alaa->text().toInt();

        bool test = voiture.modifier(VIN, newPlaqueImmatriculation, newMarque, newModele, newEtat, newCharge, newMaintenance, newDisponibilite, newAnneeFabrication, newCODE);
