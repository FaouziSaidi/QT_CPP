...
QSqlQuery qry;
int i=0;
qry.exec("SELECT FOURNISSEUR_ID FROM FOURNISSEUR");
while (qry.next())
{
    QString FOURNISSEUR_ID = qry.value(0).toString();
    i++;
    *ticks<<i;
...
QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition des FOURNISSEURS selon leurs reputations");
                  ...
                  QSqlQuery q1("SELECT REPUTATIONFOURNISSEUR FROM FOURNISSEUR");
                  while (q1.next()) {
